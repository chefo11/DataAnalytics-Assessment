...
-- This query calculates the estimated CLV for each customer and returns the result in decending order.

-- CTES to calculate the total transaction, tenure (in months) and average profit per transaction for each customer
WITH Avg_Transaction_amount_per_customer AS(SELECT u.id AS owner_id,
        ...
        COALESCE(ROUND(AVG(s.confirmed_amount) * 0.001, 2), 0) AS avg_profit_per_transaction
